...
<?php foreach($form->get_errors() as $item => $message): ?>
    <li><?php echo $item ?>: <?php echo $message ?></li>
<?php endforeach ?>
				...
				foreach ($form->type->elements as $key => $value)
				{
					switch ($value) {
						case 'module':
							...
							echo form::radio($form->type->name, $value, ( ! empty($page->target) AND $page->type === 'module')).' '.$form->type->$key->label.': ';
							echo form::dropdown($form->module->name, $form->module->values, $form->module->value).'<br />';
							break;
						
						case 'redirect':
						    if ( empty($form->redirect)) continue;
							echo form::radio($form->type->name, $value, ( ! empty($page->target) AND $page->type === 'redirect')).' '.$form->type->$key->label.': ';
							echo form::dropdown($form->redirect->name, $form->redirect->values, $form->redirect->value);
							break;
							
						default:
							echo form::radio($form->type->name, $value, empty($page->target)).' '.$form->type->$key->label;
							echo '<br />';
							break;
					}
				}
