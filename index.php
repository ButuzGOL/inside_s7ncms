...
// ----------------------------------------------------------------------------
// $Id: index.php 381 2009-05-21 21:53:36Z eduardbaun $
//
...
$s7nm_upload = file_exists($s7nm_upload) ? $s7nm_upload : DOCROOT.$s7nm_upload;
$s7nm_themes = file_exists($s7nm_themes) ? $s7nm_themes : DOCROOT.$s7nm_themes;
$s7nm_adminthemes = file_exists($s7nm_adminthemes) ? $s7nm_adminthemes : DOCROOT.$s7nm_adminthemes;

// Define application and system paths
define('APPPATH', str_replace('\\', '/', realpath($kohana_application)).'/');
define('MODPATH', str_replace('\\', '/', realpath($kohana_modules)).'/');
define('SYSPATH', str_replace('\\', '/', realpath($kohana_system)).'/');
define('UPLOADPATH', str_replace('\\', '/', realpath($s7nm_upload)).'/');
define('THEMEPATH', str_replace('\\', '/', realpath($s7nm_themes)).'/');
define('ADMINTHEMEPATH', str_replace('\\', '/', realpath($s7nm_adminthemes)).'/');

// Clean up
unset($kohana_application, $kohana_modules, $kohana_system, $s7nm_upload, $s7nm_themes, $s7nm_adminthemes);
